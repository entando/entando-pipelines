...
test_exec_cmd() {
  ...
  (
    ...
    RES="$(_exec_cmd \
      ...
      N3="$(echo "$RES" | grep -c "Error message")"
      N4="$(echo "$RES" | grep -c "Progress")"
      ...
      N1="$(echo "$RES" | grep -c "Line" "$TMPFILE")"
      N2="$(echo "$RES" | grep -c "Important Error" "$TMPFILE")"
      N3="$(echo "$RES" | grep -c "Error message" "$TMPFILE")"
