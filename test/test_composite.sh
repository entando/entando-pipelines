...
test_ppl_provision_helm_preview_environment() {
  ...
  ENTANDO_PROJECT_NAME="d02a5be3-df91-4145-ae12-c74d65977154"
  ENTANDO_PROJECT_VERSION="7.0.0-SNAPSHOT"
  ...
  (
    _ppl_provision_helm_preview_environment RES
    ASSERT RES = "test-pr11-entando-k8s-controller-coordinator"
  )

  local count="$(grep -F "{{" "$5" -c 2>/dev/null)"
  count="${count:-0}"
  ASSERT count = 0
  
  ASSERT -v CHECK_DOCKER_ORG "$(grep "$ENTANDO_OPT_DOCKER_ORG" . -r 2>/dev/null | wc -l)" = 2
  ASSERT -v CHECK_PROJECT_NAME "$(grep "$ENTANDO_PROJECT_NAME" . -r  2>/dev/null | wc -l)" = 2
  ASSERT -v CHECK_PROJECT_VER "$(grep "$ENTANDO_PROJECT_VERSION" . -r  2>/dev/null | wc -l)" = 7
