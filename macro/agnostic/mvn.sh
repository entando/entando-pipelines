    ...
    case "$action" in
      ...
      "MTX-MVN-SCAN-SONAR")
        ...
        (
          ...
          if _ppl_is_feature_action "PREVIEW-TESTS" "E"; then
            _log_i "PREVIEW TESTS ENABLED"
            export ENTANDO_OPT_PREVIEW_TESTS=true
          else
            export ENTANDO_OPT_PREVIEW_TESTS=false
          fi
      ...
      "MTX-MVN-POST-DEPLOYMENT-TESTS"|"MVN-POST-DEPLOYMENT-TESTS")
        ...
        _pp projectArtifactId projectVersion
        ...
        ENTANDO_OPT_TEST_NAMESPACE="$ENTANDO_OPT_PREVIEW_TEST_NAMESPACE"
        _ppl_run_post-deployment-test_setup_script "$ENTANDO_OPT_TEST_NAMESPACE" "$snapshotVersionName"
        [ "$?" == 99 ] && {
          _ppl_provision_helm_preview_environment ENTANDO_OPT_TEST_NAMESPACE \
            "$ENTANDO_OPT_TEST_NAMESPACE" "$snapshotVersionName"
        } || "$?" = 99 || _SOE
        ...
        (
          export ENTANDO_OPT_PREVIEW_TESTS=true
          export ENTANDO_OPT_TEST_NAMESPACE=true
