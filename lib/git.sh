...
# Checkouts a release branch for the given reference version
...
_git_checkout_release_branch() {
  ...
  __git switch "$newBranch" 2>/dev/null || git switch -c "$newBranch" || _FATAL "git switch failed"
