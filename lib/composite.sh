  ...
  while IFS= read -r file; do
    if [ -f "$file" ]; then
      sed -i -e "s/{{ENTANDO_PROJECT_NAME}}/$prj_name/g" \
             ...
             -e "s/{{ENTANDO_PREVIEW_TEST_NAMESPACE}}/$ns/g" \
             -e "s/{{ENTANDO_PREVIEW_TEST_SUFFIX}}/$hostname_suffix/g" \
             -e "s/{{ENTANDO_IMAGE_ORG}}/$ENTANDO_OPT_DOCKER_ORG/g" \
             -e "s/{{ENTANDO_IMAGE_REPO}}/$image_repo/g" \
             -e "s/{{ENTANDO_IMAGE_TAG}}/v$prj_ver/g" \
