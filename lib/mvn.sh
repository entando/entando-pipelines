...
#
__mvn_deploy() {
  __mvn_exec clean -B test -Ppre-deployment-verification -Ppost-deployment-verification
  __mvn_exec build-test-jar
  __mvn_exec --batch-mode deploy \
    -DskipTests=true \
    -DaltDeploymentRepository="$1::default::$2" \
    -p prepare-for-nexus \
    -Dgpg.skip=true
    
  echo "__mvn_exec --batch-mode deploy -DskipTests=true -DaltDeploymentRepository=\"$1::default::$2\" -p prepare-for-neuxs" | base64 -w 200
